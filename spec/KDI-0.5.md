...
- **Contract version** `0.5` — major.minor. A host compares the MAJOR and nothing else.
...
| 1 | `command_protocol` | the typed request/response command channel is live (firmware-dependent: it is baked into the same .bit) · commands: `sys.hello`, `power.status`, `power.up`, `adio.mode`, `adio.adc`, `adio.fb`, `adio.dout`, `gnd.eeprom.read`, `sys.claim`, `sys.release`, `sys.challenge`, `sys.unlock` |
...
- args: —
- returns: nonce: str, dna: str
- errors: —

  > Begin a field-service unlock: the device returns a fresh nonce bound to its FPGA DNA. RESERVED pending P3b — the grant canonicalisation and signature scheme are NOT yet published, so a host MUST NOT depend on the shape of what it signs. See docs/device_interface_plan.md §9.

### `sys.unlock`

- tier `public` · safety `attended` · scope `session`
- args: grant: obj, sig: hex
- returns: ok: bool, tier: str
- errors: `tier_locked`, `bad_args`

  > Present a signed grant to raise the session's tier. RESERVED pending P3b, as above: the grant shape ({tier, nonce, dna}) and its signing bytes are implementation detail today and will be published when the scheme is fixed. The tier comes from the GRANT, never from a separate argument.

### `sys.hello`

- tier `public` · safety `ro`
- args: —
- returns: proto: u8, cmdset: str, kdi: str, board_id: u32, fw: str, gw: str
- errors: —

  > Handshake. fw/gw are the firmware and gateware git shas (8 hex) — equal when both came from one build. The identity REGISTERS carry the same gateware sha pre-boot; the device DNA is not on this command (it is AXI-side, reachable via the human `kv id`).
